...
When a user visits a Breviare short link, the backend must issue an HTTP redirect to the destination URL. Two status codes are applicable: `301 Moved Permanently` and `302 Found` (temporary redirect). The choice has significant implications for analytics and future flexibility.
...
- **Con:** Permanent caching conflicts with Breviare's link expiry model; expired links would still redirect for users who previously visited
...
Analytics are a core feature of Breviare. The 301 caching behavior would make click tracking fundamentally unreliable after the first visit, which eliminates the primary value proposition for authenticated users. The latency trade-off is acceptable given that the redirect path is the hot path and is optimized for single-query lookup.
...
- SEO: short links will not pass link equity to destinations. This is acceptable — Breviare is a utility, not an SEO product
